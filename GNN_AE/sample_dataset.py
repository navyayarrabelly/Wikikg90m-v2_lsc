...
#         if(cand in ent_id_dict and  cand != t_ind):
#             neg_list.append(ent_id_dict[cand])
#     if(len(neg_list)<500): 
#         randomlist = random.sample(range(0, len(train_ent_set)),500-len(neg_list))
...
# torch.save(valid_dict,"/data/ogb/ogbl_wikikg2/smore_folder/sample/raw/split/time/valid.pt")
...
import random
import torch
subset_train_hrt = np.load("/data/ogb/ogbl_wikikg2/smore_folder/sample/wikikg90m-v2/processed/train_hrt.npy")
rand_indices= random.sample(range(0, subset_train_hrt.shape[0]),25000)
...
torch.save(train_eval_dict,"/data/ogb/ogbl_wikikg2/smore_folder/sample/raw/split/time/train.pt")

rel_feat = np.load("/data/ogb/ogbl_wikikg2/smore_folder/wikikg90m-v2/processed/relation_feat.npy")
torch.save(torch.tensor(rel_feat, dtype=torch.float32),"/data/ogb/ogbl_wikikg2/smore_folder/sample/raw/rel_feat.pt")
